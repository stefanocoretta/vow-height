...
```{r}
#| label: bm-1-draws-average-c1

bm_1_draws_long_avg_c1 <- bm_1_draws_long |> 
  group_by(.chain, .iteration, .draw, coef, c1_place) |> 
  summarise(epred = mean(value), .groups = "drop")

bm_1_draws_long_avg_c1 |> 
  group_by(coef, c1_place) |> 
  summarise(
...
```{r}
#| label: bm-1-draws-average-c1-vow

bm_1_draws_long_avg_c1_vow <- bm_1_draws_long |> 
  group_by(.chain, .iteration, .draw, coef, vowel, c1_place) |> 
  summarise(epred = mean(value), .groups = "drop")

bm_1_draws_long_avg_c1_vow |> 
  filter(coef == "a") |> 
  group_by(c1_place, vowel) |> 
  summarise(
    mean = mean(epred), sd = sd(epred),
    lo_90 = quantile2(epred, 0.05),
    hi_90 = quantile2(epred, 0.95)
...
```{r}
#| label: bm-1-draws-average-place-vow
...
bm_1_draws_long_avg_pl_vow |> 
  filter(coef == "a") |> 
  group_by(c1_place, c2_place, vowel) |> 
  summarise(
    mean = mean(epred), sd = sd(epred),
    lo_90 = quantile2(epred, 0.05),
    hi_90 = quantile2(epred, 0.95)
