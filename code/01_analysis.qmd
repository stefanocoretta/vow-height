...
fixef(bm_1_priors, probs = c(0.05, 0.95))
```
...
bm_1_draws_long <- bm_1_draws |> 
  select(`b_a_vowela:c1_placelabial:c2_placelabial`:`b_b_vowelu:c1_placevelar:c2_placevelar`, .chain, .iteration, .draw) |> 
  pivot_longer(`b_a_vowela:c1_placelabial:c2_placelabial`:`b_b_vowelu:c1_placevelar:c2_placevelar`) |> 
...
bm_1_coef_table_a <- bm_1 %>%
  as_draws_df() %>%
  select(`b_a_vowela:c1_placelabial:c2_placelabial`:`b_a_vowelu:c1_placevelar:c2_placevelar`) %>%
  pivot_longer(`b_a_vowela:c1_placelabial:c2_placelabial`:`b_a_vowelu:c1_placevelar:c2_placevelar`) %>%
  group_by(name) |> 
  reframe(
...
bm_1_coef_table_b_2 <- bm_1 %>%
  as_draws_df() %>%
  select(`b_b_vowela:c1_placelabial:c2_placelabial`:`b_b_vowelu:c1_placevelar:c2_placevelar`) %>%
  pivot_longer(`b_b_vowela:c1_placelabial:c2_placelabial`:`b_b_vowelu:c1_placevelar:c2_placevelar`) %>%
  group_by(name) |> 
  reframe(
    q95 = round(quantile2(value, probs = c(0.025, 0.975)), 2),
    q90 = round(quantile2(value, probs = c(0.05, 0.95)), 2),
...
conditional_effects(bm_1, "f13_z:vowel", conditions = make_conditions(bm_1, c("c1_place", "c2_place")), prob = 0.9)
```

```{r}
#| label: bm-1-cond-spag

conditional_effects(bm_1, "f13_z:vowel", conditions = make_conditions(bm_1, c("c1_place", "c2_place")), spaghetti = TRUE, ndraws = 100, prob = 0.9)
```
...
bm_1_grid <- expand_grid(
  ...
  c1_place = levels(ita_egg$c1_place),
  c2_place = levels(ita_egg$c2_place),
  speech_rate_logz = 0
)
